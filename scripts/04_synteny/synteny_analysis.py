...
"""
synteny_analysis.py
Quantifies whether flagellar gene order agrees with the GTDB phylogeny.

For each genome, gene order on its primary contig (the contig with the most
flagellar genes — same rule used by gene_order_overlay.py) is compared
pairwise against every other genome via a normalised breakpoint distance.
This is compared against patristic (phylogenetic) distance from the pruned
GTDB tree using a Mantel test, plus a per-organism agreement score (local
rank correlation between that genome's gene-order neighbours and its
phylogenetic neighbours).

Only the primary contig is used: draft assemblies split flagellar genes
across many small contigs with unknown relative order, so mixing contigs
in would inject noise into the numeric metric (visual overlays can still
show every contig — this is a scope decision for the quantitative side only).
Genes duplicated on the primary contig (paralogs) are dropped from the
ordering, since which copy corresponds to which in another genome is
ambiguous.

Run AFTER gene_order.py and gtdb_tree.py.
"""
...
OUTPUT_JSON = "campy_orthologs/archive_json/synteny_analysis.json"
